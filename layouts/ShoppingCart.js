...
export default function ShoppingCart({ cart, show, onHide, setCart }) {
  const sizeLabels = {
    XS: 'Extra Small',
    S: 'Small',
    M: 'Medium',
  ...
  return (
    <Transition
      ...
      enterFrom='opacity-0'
      enterTo='opacity-100'
      leave='duration-75 linear'
      leaveFrom='opacity-100'
      leaveTo='opacity-0'
      className='fixed inset-0 z-20 bg-gray-900/25'>
      <Transition.Child
        enter='duration-150 linear'
        ...
        className='h-full w-96 bg-white border-l shadow-md ml-auto relative'>
        <div className='p-4 pb-36 h-full overflow-y-auto scrollbar-hidden'>
          <div className='flex justify-between '>
            ...
            <XIcon
              className='w-5 text-gray-400 hover:text-gray-600 cursor-pointer'
              onClick={onHide}
            />
          </div>
          {cart.length > 0 ? (
            <>
              <ul className='divide-y py-5'>
                {groupCartProducts(cart).map(({ id, qty, color, name, price, size }, i) => (
                  <li key={i} className='space-x-2.5 py-5 flex'>
                    <div className='flex-shrink-0 w-20 h-[6.25rem]'>
                      <ProductImage width={80} path={`products/${id}/${color}/3`} />
                    </div>
                    <div className='flex-1 inline-flex flex-col justify-between'>
                      <div>
                        <div className='font-medium inline-flex justify-between w-full'>
                          <h2 className='text-gray-900 w-auto truncate'>{name}</h2>
                          <p className='pl-2'>{price} Ks</p>
                        </div>
                        <h5 className='text-xs text-gray-600 mt-2'>{sizeLabels[size] || size}</h5>
                      </div>
                      <div className='text-sm inline-flex justify-between'>
                        <p className='text-gray-600'>Qty {qty}</p>
                        <Button
                          className='font-medium text-teal-400 hover:text-teal-600 ring-transparent'
                          onClick={() =>
                            setCart(
                              cart.filter(
                                item =>
              ...
              <div className='p-4 border-t absolute bottom-0 inset-x-0 bg-white'>
                <div className='flex justify-between font-medium'>
                  <h5>Subtotal</h5>
                  <span>{clacTotalPrice(cart)} Ks</span>
                </div>
                <h6 className='text-sm text-gray-500 mt-2'>
                  Shipping and taxes calculated at checkout.
                </h6>
                <Link href='/checkout'>
                  <a className='bg-teal-600 hover:bg-teal-800 text-white w-full py-2 mt-8 block rounded-md text-center text-sm font-semibold'>
                    Checkout
                  </a>
                </Link>
              </div>
            ...
            <div className='h-full flex flex-col justify-center items-center'>
              <EmptyCart />
              <p className='mt-3 mb-6 text-gray-600'>You haven't add anything to your cart.</p>
              <Link href='/collections/all'>
                <a className='bg-teal-50 text-teal-600 hover:bg-teal-100 hover:text-teal-800 duration-300 py-1 px-3 rounded-md text-sm'>
                  Shop now
                </a>
              </Link>
            </div>
...
const EmptyCart = () => (
  <svg className='w-64 text-teal-600' viewBox='0 0 896 747.97143'>
    <path
    ...
    <polygon
      ...
      fill='#2f2e41'
    />
    <rect x='82.2584' y='458.58385' width='345.2931' height='1.7035' fill='#2f2e41' />
    <rect x='101.45894' y='521.34377' width='306.31852' height='1.7035' fill='#2f2e41' />
    <rect x='254.31376' y='402.36843' width='1.7035' height='186.53301' fill='#2f2e41' />
    <rect
      x='385.55745'
      y='570.79732'
      width='186.92877'
